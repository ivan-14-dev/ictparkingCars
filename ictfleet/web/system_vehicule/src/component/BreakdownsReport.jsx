...
import { breakdownsAPI } from '../service/api';
...
const BreakdownsReport = () => {
  ...
  const fetchBreakdowns = async () => {
    try {
      ...
      const data = await breakdownsAPI.getBreakdowns(params);
  ...
  const handleUpdateStatus = async (breakdownId, newStatus) => {
    try {
      await breakdownsAPI.updateBreakdown(breakdownId, { status: newStatus });
  ...
  const filteredBreakdowns = breakdowns.filter(breakdown => {
    ...
    return (
      breakdown.title.toLowerCase().includes(searchLower) ||
      breakdown.vehicle_info.toLowerCase().includes(searchLower) ||
      breakdown.mechanic_name.toLowerCase().includes(searchLower)
    );
  });
  ...
  return (
    <Container>
      ...
      <TableContainer>
        {loading ? (
          <LoadingSpinner>Loading breakdowns...</LoadingSpinner>
            ...
            <tbody>
              {filteredBreakdowns.map(breakdown => (
                <tr key={breakdown.id}>
                  ...
                  <td>{breakdown.vehicle_info}</td>
                  <td>
                    <strong>{breakdown.title}</strong>
                    <div style={{ fontSize: '0.85rem', color: '#64748b', marginTop: '0.25rem' }}>
                      {breakdown.description.substring(0, 100)}...
                    </div>
                  </td>
                  <td>{breakdown.mechanic_name || 'Unknown'}</td>
                  <td>
                  ...
                  <td>
                    {breakdown.image ? (
                      <ImagePreview
                        src={breakdown.image}
                        alt={breakdown.title}
                        onClick={() => setSelectedImage(breakdown.image)}
                      />
                    ) : (
                  ...
                  <td>
                    {breakdown.status === 'acknowledged' && (
                      <ResolveButton
                        onClick={() => handleUpdateStatus(breakdown.id, 'resolved')}
                      >
                        Mark Resolved
                      </ResolveButton>
                    )}
                    {breakdown.status === 'resolved' && (
                      <ActionButton
                        ...
                        onClick={() => handleUpdateStatus(breakdown.id, 'closed')}
                      >
                        Close
                      </ActionButton>
                    )}
      ...
      {selectedImage && (
        <ImageModal onClick={() => setSelectedImage(null)}>
          <img src={selectedImage} alt="Breakdown" />
        </ImageModal>
      )}

      {isDetailModalOpen && selectedBreakdown && (
            ...
            <ModalBody>
              ...
              {selectedBreakdown.image && (
                <DetailSection>
                  ...
                  <BreakdownImage 
                    src={selectedBreakdown.image}
                    alt={selectedBreakdown.title}
                    onClick={() => setSelectedImage(selectedBreakdown.image)}
...
export default BreakdownsReport;
